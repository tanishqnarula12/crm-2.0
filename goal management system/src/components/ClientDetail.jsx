    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in">
      ...
      <Card className="p-6 border border-slate-200/60 dark:border-slate-800/80 bg-white dark:bg-slate-900 shadow-md rounded-[20px] space-y-5">
        ...
        <div className="space-y-3.5">
          ...
          <RoleField label="Account Owner" value={owner} />
          <RoleField label="Portfolio Manager" value={portfolioManager} />
          <RoleField label="Service Manager" value={serviceManager} />
          <RoleField label="Insurance Manager" value={insuranceManager} />
        </div>
      </Card>
              ...
              familyDetails.map((f, i) => (
                <div key={i} className="py-2.5 flex items-center justify-between text-xs">
                  <div className="flex flex-col">
                    ...
                    {f.dob && <span className="text-[10px] text-slate-400 dark:text-slate-500 font-sans mt-0.5">DOB: {fmtDate(f.dob)}</span>}
                  </div>
                  <span className="text-[9px] font-bold uppercase tracking-wider bg-slate-100 dark:bg-slate-800/60 text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded border border-slate-200/20">{f.relation}</span>
                </div>
              ))
            )}
          </div>
